...
| `05_full_scale_sft_t4.ipynb` | **Max T4-scale run**: HF stream (400/200/150) → 3k–6k pairs → full-epoch 3B QLoRA → optional RAG → **model card + HF upload** → **GitHub metadata push** |
...
`05_full_scale_sft_t4.ipynb` is the **scale** path after smoke corpus (N=4). Defaults: `DOWNLOAD_HF=True`, catalog caps (400/200/150), `MAX_STEPS=None` (1 epoch), `RUN_TRAIN=False` until you flip it on a T4. After train:

- `PUBLISH_HF=True` → `scripts/write_model_card.py` + `scripts/publish_adapter.py --run` (adapter + filled README model card)
- `PUSH_GITHUB=True` → `scripts/push_run_metadata.py --run` (sft_plan, manifests, metrics only — no weights)

Expect multi-hour wall time; copy the adapter off the session before it dies.
...
python scripts/push_run_metadata.py              # dry-run list
python scripts/push_run_metadata.py --run        # needs GITHUB_TOKEN
```
...
### GitHub token (notebook 04 / 05 push)
